    ...
    >
      <template #actions>
        <button
          @click="handleExport"
          type="button"
          class="text-xs text-green-700 px-2 py-1 rounded border border-green-300 hover:bg-green-50"
          data-testid="export-btn-mobile"
        >导出</button>
        <button
          @click="$refs.importInput.click()"
          :disabled="importing"
          type="button"
          class="text-xs text-blue-600 px-2 py-1 rounded border border-blue-300 hover:bg-blue-50 disabled:opacity-50"
          data-testid="import-btn-mobile"
        >{{ importing ? '...' : '导入' }}</button>
        <input ref="importInput" type="file" accept=".json" class="hidden" @change="handleImport" />
      </template>
    </AppHeader>

    <div class="pt-14 lg:pt-0 p-4 lg:p-6">
      <!-- Desktop-only header -->
      <div class="hidden lg:flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-2">
          ...
          >导出排阵</button>
          <button
            @click="$refs.importInputDesktop.click()"
            :disabled="importing"
