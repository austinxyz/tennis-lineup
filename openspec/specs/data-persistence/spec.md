...
- **THEN** system reads JSON file from the configured core data path (`storage.data-file`, default `./data/tennis-data.json`)
...
The system SHALL read configuration data (constraint presets, etc.) from a separate config JSON file (`tennis-config.json`). This file SHALL use an independent `ReadWriteLock` from the core data file.

#### Scenario: Successful config read
- **WHEN** system needs to load configuration data
- **THEN** system reads JSON file from the configured config path (`storage.config-file`, default `./data/tennis-config.json`)
...
- **THEN** system creates the file with empty ConfigData on startup
...
- **THEN** system uses temp file + atomic rename (same pattern as core data write)
...
The system SHALL read both file paths from `application.yml` with sensible defaults.
