...
The system SHALL allow users to save named constraint configurations (excludePlayers, includePlayers, pinPlayers) as presets per team. Presets SHALL be persisted in the team's data and available across sessions.
...
- **AND** the server SHALL persist the preset with a generated `id` ("preset-" + nanoTime) and `createdAt` timestamp and return HTTP 201 with the created preset
...
- **AND** the server SHALL remove the preset and return HTTP 204
