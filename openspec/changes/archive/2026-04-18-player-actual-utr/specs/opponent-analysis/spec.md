...
### Requirement: Win probability uses effective actualUtr
The per-line win probability calculation SHALL use each player's effective actualUtr (`actualUtr ?? utr`) instead of official `utr` when computing the UTR delta between matched pairs. Hard constraint checks (lineup validity, total UTR cap) continue to use official `utr`.

#### Scenario: Win probability computed from actualUtr delta
- **WHEN** `POST /api/lineups/matchup` is called
- **THEN** the per-line expected score SHALL be computed using the effective actualUtr of each player in the pair
- **THEN** if a player has `actualUtr: null`, `utr` is used as the effective actualUtr

#### Scenario: No player has actualUtr set — results unchanged
- **WHEN** all players in both lineups have `actualUtr: null`
- **THEN** per-line win probability results SHALL be identical to the pre-change behavior
...
#### Scenario: AI prompt includes actualUtr instruction
- **WHEN** AI analysis is triggered
- **THEN** the AI prompt SHALL include the instruction: "请主要参考实际UTR（actualUtr）进行分析。如果实际UTR与官方UTR不同，以实际UTR为准。同时结合个人备注和搭档笔记进行综合判断。"
- **THEN** player roster data sent to AI SHALL include effective actualUtr values alongside official UTR
